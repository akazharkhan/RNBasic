      ...
      POST(
        ...
        res => {
          ...
          if (res.ok) {
            ...
            navigation.navigate("OtpVerify", { mobile_number })
          } else {
    ...
    <ViewContainer style={{ flex: 1, backgroundColor: colors.darkcyan }}>
      <ScrollContainer>
        ...
        <View style={styles.BoxTWOContainer}>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between',backgroundColor:'#fff5ee',paddingVertical:6,borderRadius:10 }}>
            <CountryPicker
              disable={false}
              animationType={"slide"}
              language="en"
              containerStyle={[styles.pickerStyle, { paddingHorizontal: 10,backgroundColor:'#fff5ee'}]}
            ...
            />
            <Input onChange={setMobileNumber} error={errors?.mobile_number} style={{backgroundColor: '#fff5ee',borderLeftWidth:1,borderColor:'gray',width:'70%' }} placeholder='Mobile Number' />
          </View>
          <Input onChange={setPassword} error={errors?.password} style={{ backgroundColor: '#fff5ee',marginTop:20 }} placeholder='Password' />
